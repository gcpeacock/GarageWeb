...
LOG_FILE="/home/pi/GarageWeb/backend/garagewebapi/static/log.txt"

#Set up logger to file and console
logger = logging.getLogger("GARAGELOG")
...
rotation = TimedRotatingFileHandler(LOG_FILE,
                                    when="d",
                                    interval=1,
                                    backupCount=7)
formatter2 = logging.Formatter('%(asctime)-6s %(levelname)-8s %(message)s')
rotation.setFormatter(formatter2)
...
logger.info("Program Starting -- Hello!")
...
class GarageDoor:
    def __init__(self, DoorName, pin1, pin2):
        ...
        logger.debug(self.DoorName + "'s Garage Door initialized on pins + " + str(self.pin1) + " and " + str(self.pin2))
    ...
    def run(self, _running):
        while _running():
            ...
            if self.DoorOpenTimer == 1:  #Door Open Timer has Started
                ...
                if (currentTimeDate - self.TimeDoorOpened).seconds > 900 and self.DoorOpenTimerMessageSent == 0:
                    logger.info(self.DoorName + "'s Garage Door has been Open for 15 minutes")
            ...
            if GPIO.input(self.pin1) == GPIO.HIGH and GPIO.input(self.pin2) == GPIO.HIGH:  #Door Status is Unknown
                logger.info(self.DoorName + "'s Door is Opening/Closing")
                while GPIO.input(self.pin1) == GPIO.HIGH and GPIO.input(self.pin2) == GPIO.HIGH:
                    ...
                    if GPIO.input(self.pin1) == GPIO.LOW:  #Door is Closed
                        logger.info(self.DoorName + "'s Door is Closed")
                        self.DoorOpenTimer = 0

                    if GPIO.input(self.pin2) == GPIO.LOW:  #Door is Open
                        logger.info(self.DoorName + "'s Door is Open")
        ...
        if _running() == False:
            logger.debug(self.DoorName + " terminating")
...
try:
    ...
    logger.info("Shutting down the door monitors")
    while threading.active_count() > 1:
        logger.info(threading.active_count())
        time.sleep(.5)
    logger.info("Program Shutdown -- Goodbye!")
    GPIO.cleanup()
